...
public interface EntryDao {
  ...
  @Transaction @Query("SELECT id, primary_kanji, primary_reading FROM entries WHERE primary_kanji LIKE :term LIMIT 20")
  DataSource.Factory<Integer, SearchResultEntry> searchByKanji(String term);

  @Transaction @Query("SELECT id, primary_kanji, primary_reading FROM entries WHERE primary_reading LIKE :term LIMIT 20")
  DataSource.Factory<Integer, SearchResultEntry> searchByReading(String term);

  @Transaction @Query("SELECT entries.id, entries.primary_kanji, entries.primary_reading FROM entries JOIN senses ON senses.entry_id = entries.id WHERE senses.glosses LIKE :term LIMIT 20")
  ...
  @Transaction @Query("SELECT id, primary_kanji, primary_reading FROM entries WHERE jlpt_level = :level ORDER BY id ASC")
  DataSource.Factory<Integer, SearchResultEntry> findByJlptLevel(Integer level);

  // WIDGET

  @Transaction @Query("SELECT id, primary_kanji, primary_reading FROM entries WHERE jlpt_level = :level ORDER BY id ASC LIMIT 1 OFFSET :offset")
